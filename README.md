...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `george` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `GERey` with the `ORG` of your provider.
   - replace all instances of the `github.com/pulumi/pulumi-george` repository with the `REPOSITORY` location
...
├── config.yaml
├── metadata.json
├── pulumi-george_darwin_amd64_v1
│   └── pulumi-resource-george
├── pulumi-george_darwin_arm64
│   └── pulumi-resource-george
├── pulumi-george_linux_amd64_v1
│   └── pulumi-resource-george
├── pulumi-george_linux_arm64
│   └── pulumi-resource-george
├── pulumi-george_windows_amd64_v1
│   └── pulumi-resource-george.exe
└── pulumi-george_windows_arm64
    └── pulumi-resource-george.exe
```
